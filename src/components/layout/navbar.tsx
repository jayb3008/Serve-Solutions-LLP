  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center">
          <div className="flex items-center">
            <Link to="/" className="flex items-center">
              <span className="text-2xl font-bold gradient-text">Serve<span className="text-brand-darkBlue">Solutions</span></span>
            </Link>
          </div>
          ...
          <div className="hidden md:flex items-center space-x-8">
            {navigationItems.map((item) => (
              <div key={item.name} className="relative group">
                {item.submenu ? (
                  <button 
                    ...
                    className={`flex items-center text-gray-700 dark:text-gray-200 hover:text-brand-blue transition-colors ${
                      location.pathname === item.href ? 'text-brand-blue font-medium' : ''
                    }`}
                  >
                    {item.name}
                ...
                {item.submenu && (
                  <div className={`absolute left-0 mt-2 w-48 origin-top-left bg-white dark:bg-gray-800 rounded-md shadow-lg ring-1 ring-black ring-opacity-5 transition-all z-50 ${
        ...
        {/* Mobile Navigation */}
        <div className={`md:hidden transition-all duration-300 ease-in-out ${isOpen ? 'max-h-screen opacity-100 mt-4' : 'max-h-0 opacity-0 overflow-hidden'}`}>
              ...
              <div key={item.name}>
                {item.submenu ? (
                  <>
                    ...
                    >
                      <span>{item.name}</span>
                      <ChevronDown className={`h-4 w-4 transition-transform ${openSubmenu === item.name ? 'transform rotate-180' : ''}`} />
                    </button>
                    <div className={`pl-4 space-y-2 transition-all duration-300 ${openSubmenu === item.name ? 'max-h-screen opacity-100 mt-2' : 'max-h-0 opacity-0 overflow-hidden'}`}>
                      {item.submenu.map((subitem) => (
                        <Link
                          key={subitem.name}
                          to={subitem.href}
